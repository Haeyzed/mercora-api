...
Enforce where consumers exist: maintenance middleware, API throttle, Sanctum timeout, lockout window, locale/timezone, activitylog clean, notice channels, domain subdomain/custom policy, concurrent provisions, auto_provision, invoice memo, payment statement title, thumbnails, PII mask. Display-only and no-consumer keys stay schema-only until a feature lands.
