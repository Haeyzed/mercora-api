<?php

declare(strict_types=1);

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('invoices', function (Blueprint $table) {
            $table->unsignedInteger('subtotal')->nullable()->after('amount');
            $table->decimal('tax_rate', 8, 4)->nullable()->after('subtotal');
            $table->unsignedInteger('tax_amount')->default(0)->after('tax_rate');
            $table->boolean('tax_inclusive')->default(false)->after('tax_amount');
            $table->json('seller')->nullable()->after('notes');
        });
    }

    public function down(): void
    {
        Schema::table('invoices', function (Blueprint $table) {
            $table->dropColumn(['subtotal', 'tax_rate', 'tax_amount', 'tax_inclusive', 'seller']);
        });
    }
};
